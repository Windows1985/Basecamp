...
## Context
My home router is an ASUS RT-BE series (WiFi 7 / 802.11be). The ESP32-S3 firmware stack expects standard 802.11n behaviour on 2.4GHz. WiFi 7 introduces BSS colouring, MU-MIMO enhancements, and channel management features that interfere with CSI capture.
...
## Consequences
WiFi 7 + ESP32-S3 CSI capture is untested in the RuView community. If CSI data is still corrupted after applying these settings, the fallback is a dedicated travel router (~80HKD). This is an acceptable risk given the low cost of the fallback.
