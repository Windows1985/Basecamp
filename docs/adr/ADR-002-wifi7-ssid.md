...
## Consequences
WiFi 7 + ESP32-S3 CSI capture is untested in the RuView community. If CSI data is still corrupted after applying these settings, the fallback is a dedicated travel router (~80 RMB). This is an acceptable risk given the low cost of the fallback.
