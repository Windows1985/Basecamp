...
## Reasoning
A fixed time does not account for variable wake times. If I sleep until 9am, a 7am batch job runs on incomplete data. If I wake at 6am, I have to wait an hour for the report. The radar already runs overnight for presence detection, so bed exit is a zero-cost signal. The 4-hour minimum prevents bathroom trips at 3am from triggering the batch.
